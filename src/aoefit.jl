...
"""
function fit_aoe_compton(peakhists::Array, peakstats::StructArray, compton_bands::Array{T},; pars_aoe::NamedTuple{(:μ, :μ_err, :σ, :σ_err)}) where T<:Real
    if isempty(pars_aoe)
...
#                 B = weibull_from_mx(ps.mean_background, 3*ps.mean_background),
