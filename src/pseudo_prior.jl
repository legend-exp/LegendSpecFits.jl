        ...
        window_right = maximum(h.edges[1]) - ps.peak_pos
         return NamedTupleDist(
             ...
             background_slope = ifelse(ps.mean_background < 5, # mean_background just an estimate; cannot risk negative counts
                        ConstValueDist(0), truncated(Normal(0, ps.mean_background_std / (window_left + window_right)), - ps.mean_background / window_right, (ps.mean_background + ps.mean_background_step) / window_left)),
         )
